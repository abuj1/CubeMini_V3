...
bool mpu6000_spi::init(int sample_rate_div,int low_pass_filter){
    ...
    // ENABLE GYROACC
		ThisThread::sleep_for(chrono::milliseconds(100));
		select();
    response=spi.write(0x68);
    response=spi.write(0x07);
    deselect();
		ThisThread::sleep_for(chrono::milliseconds(100));
    
    
		init(1,BITS_DLPF_CFG_42HZ);
    set_acc_scale(BITS_FS_2G);
    set_gyro_scale(BITS_FS_500DPS);
	return true;
}

bool mpu6000_spi::init_inav(){
 unsigned int response;
    spi.format(8,0);
    spi.frequency(1000000);
    ...
    write2spi(0x19,0x02);
		ThisThread::sleep_for(chrono::milliseconds(15));
    write2spi(0x1B,0x03<<3);
		ThisThread::sleep_for(chrono::milliseconds(15));
    write2spi(0x1C,0<<3);
		ThisThread::sleep_for(chrono::milliseconds(15));
    write2spi(0x37,0 << 7 | 0 << 6 | 0 << 5 | 1 << 4 | 0 << 3 | 0 << 2 | 0 << 1 | 0 << 0);
		ThisThread::sleep_for(chrono::milliseconds(15));
    write2spi(0x38,1<<0);
		ThisThread::sleep_for(chrono::milliseconds(15));
...
float mpu6000_spi::readAcc(int axis){
...
void mpu6000_spi::readAccTempGyro(void){
    uint8_t response[16];
    int16_t bit_data;
    select();
...
float mpu6000_spi::readGyro(int axis){
    uint8_t responseH,responseL;
    int16_t bit_data;
    float data;
    select();
...
void mpu6000_spi::readGyro(void){
    uint8_t responseHx,responseLx,responseHy,responseLy,responseHz,responseLz;
    int16_t bit_data;
    float data;
    select();
...
void mpu6000_spi::write2spi(uint8_t reg,uint8_t val){
	  unsigned int response;
		cs = 0;
